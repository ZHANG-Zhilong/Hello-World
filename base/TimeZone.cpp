...
server::TimeZone::TimeZone(const char* zonefile)
...
server::TimeZone::TimeZone(int eastOfUtc, const char* name)
        : data_(new TimeZone::Data)
{
...
struct tm server::TimeZone::toLocalTime(time_t seconds) const
{
    struct tm localTime;
...
time_t server::TimeZone::fromLocalTime(const struct tm& localTm) const
{
    assert(data_ != NULL);
    const Data& data(*data_);
...
struct tm server::TimeZone::toUtcTime(time_t secondsSinceEpoch, bool yday)
{
    struct tm utc;
...
time_t server::TimeZone::fromUtcTime(const struct tm& utc)
{
    return fromUtcTime(utc.tm_year + 1900, utc.tm_mon + 1, utc.tm_mday,
                       utc.tm_hour, utc.tm_min, utc.tm_sec);
}

time_t server::TimeZone::fromUtcTime(int year, int month, int day,
